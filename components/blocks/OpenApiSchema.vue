<template>
  <div class="schema border border-gray-300 rounded p-5 mb-10">
    ...
    <div class="schema-row flex items-center" v-if="props.schema.allOf">
...
const additionalPropertiesRef = computed(() => {
  ...
  let res = [];
  for (let i in props.schema.additionalProperties) {
    let param = props.schema.additionalProperties[i]
    if (param.$ref) {
