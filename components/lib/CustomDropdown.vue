<template>
  <div class="relative inline-block">
    <div class="dropdown-toggle flex items-center justify-between px-4 py-2 border border-gray-300 rounded-md cursor-pointer" @click="isOpen = !isOpen">
      <span class="text-gray-500 dark:text-gray-300/75">{{ selectedOption || placeholder }}</span>
    ...
    <div class="dropdown-menu absolute top-full left-0 z-50 py-2 bg-white rounded-md shadow-lg dark:bg-black dark:text-gray-300/75" v-if="isOpen">
      <div v-for="(option, index) in options" :key="index" class="dropdown-item px-4 py-2 hover:bg-gray-100 dark:text-gray-300 dark:hover:text-gray-900" :class="{'bg-gray-300 dark:text-gray-900': option === selectedOption}" @click.stop.prevent="selectOption(option)">
