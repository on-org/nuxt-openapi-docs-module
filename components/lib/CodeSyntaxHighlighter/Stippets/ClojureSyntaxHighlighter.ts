...
export default class ClojureSyntaxHighlighter extends SyntaxHighlighter {
  constructor() {
    const language: Language = {
      name: 'clojure',
      aliases: ['clj'],
      keywords: [
        'def', 'defn', 'defmacro', 'defmethod', 'defmulti', 'defproject', 'defrecord', 'defstruct',
        'do', 'if', 'let', 'loop', 'when', 'while', 'cond', 'case', 'when-let', 'doseq', 'dotimes'
      ],
      operators: [
        '+', '-', '*', '/', '%', '==', '!=', '>', '<', '>=', '<=', 'and', 'or', 'not'
      ],
