<template>
  <div class="examples-container">
    <h4 class="text-xl font-medium mb-2">{{ $openapidoc.getLocaleText(currentLocale, 'Examples') }}:</h4>
    <div v-for="(example, exampleName) in examples" :key="exampleName">
      ...
      <div class="example" v-if="example">
        <template v-if="example.summary">
          <p class="mb-1"><strong>{{ $openapidoc.getLocaleText(currentLocale, 'Summary') }}:</strong> {{ tr(example, 'summary', currentLocale) }}</p>
        </template>
        ...
        <template v-if="example.value">
          <OpenApiSchema :schema="example.schema" :current-locale="currentLocale" :components="components" />
