...
interface OperationObject30 {
  ...
  responses: ResponsesObject; // Описание возможных ответов для этой операции
...
interface ResponsesObject {
  default?: ResponseObject | ReferenceObject | undefined; // Описание ответа по умолчанию для всех статусных кодов, не указанных явно в этом объекте
  [status: string]: ResponseObject | ReferenceObject | undefined; // Каждый ключ является строкой, представляющей числовой HTTP-статусный код, а значение — объект, представляющий ответ на этот статусный код
}
